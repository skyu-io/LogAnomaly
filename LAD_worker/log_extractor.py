...
def request_json_and_trace(
    ...
    *,
    ...
    if resp.status_code != 200:
        # Include a small body preview on failure to help debugging
...
def get_org_info(session: requests.Session, url: str, token: str, orgid: str) -> Any:
    ...
    Prints start/success/error messages for simple status visibility.
    """
    t0 = time.time()
    print(f"[get_org_info] started (orgid={orgid}, url={url})")

    headers = build_headers(token, orgid, None)
    try:
        resp = session.get(url, headers=headers, timeout=HTTP_TIMEOUT)
    except requests.RequestException as e:
        print(f"[get_org_info] network error after {time.time()-t0:.2f}s: {e}")
        raise

    if resp.status_code != 200:
        print(f"[get_org_info] http {resp.status_code} after {time.time()-t0:.2f}s")
        raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:500]}")

    try:
        data = resp.json()
        print(f"[get_org_info] success in {time.time()-t0:.2f}s")
        return data
    except Exception as e:
        print(f"[get_org_info] json parse error after {time.time()-t0:.2f}s: {e}")
        raise RuntimeError(f"Failed to parse JSON: {e}\nBody starts with: {resp.text[:500]}")

def get_applications(session: requests.Session, url: str, token: str, orgid: str, projectid: str) -> Any:
    start = time.time()
    print(f"[get_applications] Started (orgid={orgid}, projectid={projectid}, url={url})")
    ...
    resp = session.get(url, headers=headers, timeout=HTTP_TIMEOUT)
    if resp.status_code != 200:
        print(f"[get_applications] HTTP error {resp.status_code} after {time.time()-start:.2f}s")
        raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:500]}")

    try:
        data = resp.json()
        print(f"[get_applications] Success in {time.time()-start:.2f}s")
        return data
    except Exception as e:
        print(f"[get_applications] JSON parse failed after {time.time()-start:.2f}s: {e}")
        raise RuntimeError(f"Failed to parse JSON: {e}\nBody starts with: {resp.text[:500]}")

def get_clusters(session: requests.Session, url: str, token: str, orgid: str, projectid: str) -> Any:
    t0 = time.perf_counter()
    print(f"[get_clusters] started (orgid={orgid}, projectid={projectid}, url={url})")
    ...
    try:
        ...
        print(f"[get_clusters] network error after {time.perf_counter()-t0:.2f}s: {e}")
        raise

    if resp.status_code != 200:
        print(f"[get_clusters] http {resp.status_code} after {time.perf_counter()-t0:.2f}s")
        raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:500]}")

    try:
        data = resp.json()
        print(f"[get_clusters] success in {time.perf_counter()-t0:.2f}s")
        return data
    except Exception as e:
        print(f"[get_clusters] json parse error after {time.perf_counter()-t0:.2f}s: {e}")
        raise RuntimeError(f"Failed to parse JSON: {e}\nBody starts with: {resp.text[:500]}")
...
def get_logs_query(
    ...
    *,
    ...
    t0 = time.perf_counter()
    print(
        "[get_logs_query] started to fetch Logs "
    ...
    try:
        result = request_json_and_trace(
            session, url, headers, params, trace_steps, label="GET /logs/query"
        )
        # Optional: give a tiny hint of result size without leaking content
        size_hint = (
            f"len={len(result)}" if isinstance(result, (list, dict, tuple, set)) else type(result).__name__
        )
        # print(f"[get_logs_query] success in {time.perf_counter()-t0:.2f}s ({size_hint})")
        return result
    except requests.RequestException as e:
        print(f"[get_logs_query] network error after {time.perf_counter()-t0:.2f}s: {e}")
        raise
    except Exception as e:
        print(f"[get_logs_query] error after {time.perf_counter()-t0:.2f}s: {e}")
        raise
...
def main():
    ...
    def run_logs_pair(project_id: str, cluster_summary: Dict[str, Any], app_id: str) -> None:
        # If cluster summary is missing or incomplete, just print and skip
        ...
        # Trace container for single consolidated log line later
        trace_steps: List[Dict[str, str]] = []
        ...
        try:
            ...
            # Print one consolidated message with what we have so far
            # print(
            #     "[CURL TRACE] flow failed at /logs/query for "
            #     f"app={app_id} project={project_id} cluster={cluster_name}\n"
            #     + "\n".join(f"- {s['label']} -> HTTP {s['status']}\n  {s['curl']}" for s in trace_steps)
            # )
            print(f"[WARN] /logs/query failed: {e}", file=sys.stderr)
            return

        query_id = extract_query_id_from_query_resp(q_json)
        if not query_id:
            # print(
            #     "[CURL TRACE] no queryId returned for "
            #     f"app={app_id} project={project_id} cluster={cluster_name}\n"
            #     + "\n".join(f"- {s['label']} -> HTTP {s['status']}\n  {s['curl']}" for s in trace_steps)
            # )
            print(
        ...
        try:
            ...
            # print(
            #     "[CURL TRACE] flow failed at /logs/logs for "
            #     f"app={app_id} project={project_id} cluster={cluster_name}\n"
            #     + "\n".join(f"- {s['label']} -> HTTP {s['status']}\n  {s['curl']}" for s in trace_steps)
            # )
            print(f"[WARN] /logs/logs failed: {e}", file=sys.stderr)
            return
        ...
        logs_aggregated.append({
            ...
            "results": results
               ...
               # Only write the file if there are actual results
        if not results:
            # print(f"[INFO] No logs for project {project_id}, cluster {cluster_name}, app {app_id} — skipping file write")
            print(f"[INFO] No logs for app {app_id} — skipping file write")
            return
        ...
        # ---- Single consolidated message AFTER both curls completed successfully
        # print(
        #     "[CURL TRACE] logs flow completed for "
        #     f"app={app_id} project={project_id} cluster={cluster_name}\n"
        #     + "\n".join(f"- {s['label']} -> HTTP {s['status']}\n  {s['curl']}" for s in trace_steps)
        # )
